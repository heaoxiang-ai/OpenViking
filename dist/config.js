...
function resolveSecret(value, label) {
    ...
    switch (obj.source) {
        ...
        case "exec": {
            const provider = typeof obj.provider === "string"
                ? obj.provider
                : "";
            if (!provider) {
                throw new Error(`OpenViking ${label} SecretRef exec source requires a "provider" field naming the secret-manager CLI on PATH`);
            }
            try {
                ...
                const msg = err instanceof Error ? err.message : String(err);
                throw new Error(`OpenViking ${label} SecretRef exec source: provider "${provider}" failed for id "${id}" (${msg})`);
            }
        }
        default:
            throw new Error(`OpenViking ${label} SecretRef has unknown source "${String(obj.source)}". Supported: "env" | "file" | "exec".`);
    }
}
